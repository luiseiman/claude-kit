...
A `trading` stack was created during a real project session. It follows dotforge stack conventions correctly (rules/, settings.json.partial, plugin.json, skills/) with no application code — only Claude Code configuration.
...
2. **Precedent**: If accepted, this opens the door for other domain-specific stacks (healthcare, e-commerce, etc.). Is that the direction dotforge should go?
